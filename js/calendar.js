...
function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        dayBookings.slice(0, 3).forEach(b => {
            ...
            html += `<div class="cal-event" style="background:${color}20;color:${color};border-left:3px solid ${color};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:0.75rem;padding:2px 4px;margin-bottom:2px;border-radius:2px;cursor:pointer" onclick="event.stopPropagation();showBookingDetail('${b.id}')">${b.nama_penyewa}</div>`
        });
